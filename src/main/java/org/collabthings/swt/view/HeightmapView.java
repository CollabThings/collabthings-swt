...
public class HeightmapView extends CTComposite implements CTAppControl {
	...
	public HeightmapView(Composite c, CTApp app, AppWindow nwindow, CTHeightmap o, boolean b) {
		...
		applicationtext.setText("" + hm.getApplication());
	...
	private void doSave(String sapplicationtext) {
		if (sapplicationtext != null && (hm.getApplication() == null || !hm.getApplication().equals(sapplicationtext))) {

			String oldapplication = hm.getApplication();
			getDisplay().asyncExec(() -> {
				hm.setApplication(sapplicationtext);
				if (hm.isOK()) {
					if (bottomtext != null) {
					...
					hm.setApplication(oldapplication);
