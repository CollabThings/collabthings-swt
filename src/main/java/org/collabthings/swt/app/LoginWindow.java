...
public class LoginWindow {
	...
	private void startLoginCheck() {
		final Thread t = new Thread(() -> {
			if (!app.getWClient().trySavedSession()) {
				WClient client = app.getWClient();
				synchronized (app) {
					loop(client);
				}
			}
	...
	public AppLoginVO getApplogin() {
		if (applogin == null) {
			...
			log.info("client:" + app.getWClient());
			this.applogin = app.getWClient().requestAppLogin();
